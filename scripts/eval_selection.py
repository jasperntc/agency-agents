...
Every responses file records a sha256 of cases.jsonl, and scoring refuses to run
if it no longer matches, so picks collected against one benchmark can never be
quietly scored against a different one.
"""
...
def cases_digest(raw: bytes) -> str:
...
def load_runs(digest: str) -> list[dict]:
    """Every recorded run, oldest name first. Refuses stale picks.

    A responses file records the sha256 of the cases file it was collected
    against. Scoring picks against an edited benchmark would silently
    re-attribute answers to questions that were never asked -- so it is a hard
    error rather than a warning.
    """
    runs = []
    for path in sorted(RESPONSES.glob("*.json")):
        run = json.loads(path.read_bytes().decode("utf-8"))
        if run.get("cases_sha256") != digest:
            raise SystemExit(
                f"{path.relative_to(REPO_ROOT).as_posix()}: recorded against a "
                f"different eval/routing/cases.jsonl.\n"
                f"  recorded {run.get('cases_sha256', '(none)')[:16]}\n"
                f"  current  {digest[:16]}\n"
                f"These picks answer different questions than the current cases "
                f"ask. Re-run the eval, or restore the cases file.")
        run["_name"] = path.stem
        runs.append(run)
    return runs


def score_run(run: dict, cases: list[dict], reachable: dict[str, bool]) -> dict:
...
def build_report() -> dict:
    ...
    return {
        ...
        "cases": {"total": len(cases), "sha256": digest},
        "literal_reachability_pct": routing["literal_reachability"]["pct"],
        "runs": [score_run(r, cases, reachable) for r in runs],
    }
...
def main() -> int:
    ...
    if args.prompt:
        cases, _ = load_cases()
        case = next((c for c in cases if c["case"] == args.prompt), None)
        if case is None:
